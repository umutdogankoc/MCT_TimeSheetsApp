...
var addRowTable = {
    ...
    rowSave: function ($row) {
        ...
        $row.children("td").each(function (i) {
            ...
            if ($this.hasClass("timeSheetLineHours")) {
                modelData.HeaderNo = $this.attr('headerId');
                modelData.LineNo = $this.attr('lineId');
                modelData.Date = $this.attr('DateId');
                modelData.Value = $this.find("input").val();
                model.push(modelData);
            }
        });
        UpdateResourceTimeSheetLine(model);
    },
